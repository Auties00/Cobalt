...
public class StanzaEncoder{
    ...
    private boolean writeStringFallback(String input) {
        return TOKENS.stream()
                .map(tokens -> tokens.indexOf(input))
                .filter(index -> index != -1)
                .peek(this::writeStringFallback)
                .findAny()
                .isPresent();
    }

    private void writeStringFallback(int fallbackTokenIndex) {
        binary.writeUInt8(STRING_TOKENS[fallbackTokenIndex]);
        binary.writeUInt8(fallbackTokenIndex);
    }
    ...
    private void writeNodeContent(Node input) {
        if (!(input.content() instanceof List<?> contentAsList)) {
            write(input.content());
            return;
        }

        writeInt(contentAsList.size());
    ...
    private void writeInt(int size) {
        ...
        if (size < UNSIGNED_SHORT_MAX_VALUE) {
            binary.writeUInt8(UNSIGNED_SHORT_TAG);
    ...
    private int nodeSize(Node input) {
        var size = 1;
        if(!input.attrs().isEmpty()){
            size += 2 * input.attrs().size();
        }
    ...
    public Object read(boolean raw) {
        ...
        return switch (token) {
            ...
            case 252 -> readString(raw);
            case 253 -> readString16(raw);
            case 254 -> readString32(raw);
            case 255 -> readNumericString();
            default -> readStringFallback(token);
        };
    }
    ...
    private String readStringFallback(int token) {
        ...
        if (token >= 236) {
            ...
            var index = binary.readUInt8();
            var value = dict.get(index);
            if (value == null) {
                throw new RuntimeException("Invalid value index ${index} in dict ${n - 236}");
            }

            return value;
        }

        var singleToken = SINGLE_BYTE.get(token - 1);
        if(singleToken == null){
            throw new RuntimeException("Undefined token with index ${n}");
        }
    ...
    private Object readString32(boolean raw) {
        if (raw) {
    ...
    private Object readString16(boolean raw) {
    ...
    private Object readString(boolean raw) {
        if (raw) {
    ...
    public byte[] encodeStanza(Node node) {
        write(node);
        var binaryArr = binary.readAllBytes();
