...
public final class MutationEncoder {
    public static EncryptedMutation encode(
        ...
        var actionData = new ActionDataSyncBuilder()
                .index(patch.index().getBytes(StandardCharsets.UTF_8))
                .value(patch.sync())
                .padding(padding)
        ...
        var operation = patch.operation().content();
        ...
        // 5. Compute index MAC
        var indexBytes = patch.index().getBytes(StandardCharsets.UTF_8);
        mac.init(keys.indexKey());
        var indexMac = mac.doFinal(indexBytes);

        // 8. Create EncryptedMutation
        return new EncryptedMutation(indexMac, encryptedValue, keyId, patch.operation());
    }
}
