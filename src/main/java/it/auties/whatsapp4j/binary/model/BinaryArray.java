package it.auties.whatsapp4j.binary.model;
...
public record BinaryArray(byte[] data) {
     ...
     * Constructs a new non empty {@code BinaryArray}
     *
     * @param in     the int to wrap
     * @param length the length of the array
     * @return a new non empty {@code BinaryArray} wrapping a bytes array that only contains {@param in}
     */
    public static @NonNull BinaryArray forInt(int in) {
        var buffer = ByteBuffer.allocate(4);
        buffer.putInt(in);
        return forArray(buffer.array());
    }
    ...
    public @NonNull BinaryArray slice(int start, int end) {
        System.err.printf("(%s)->(%s) |%s| %n", start >= 0 ? start : size() + start, end >= 0 ? end : size() + end, size());
     ...
     */
    public @NonNull BinaryArray merged(byte @NonNull [] array) {
     ...
     */
    public int toInt() {
        Validate.isTrue(size() < 5, "Cannot convert BinaryArray to int, overflow: %s".formatted(size()));
        var value = 0;
     ...
     */
    public @NonNull BinaryMessage toMessage(boolean multiDevice) {
        if(multiDevice){
            return new MultiDeviceBinaryMessage(this);
        }

        var index = indexOf(StandardBinaryMessage.DIVIDER)
                .orElseThrow(() -> new IllegalArgumentException("Cannot convert BinaryArray to BinaryMessage"));
        var tag = cut(index);
        var message = slice(index + 1);
        return new StandardBinaryMessage(tag.toString(), message);
    }
