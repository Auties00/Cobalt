...
public record SessionBuilder(@NonNull SessionAddress address, @NonNull Keys keys)
  ...
  public void createOutgoing(int id, byte[] identityKey, SignalSignedKeyPair signedPreKey,
    ...
    Validate.isTrue(
        Curve25519.verifySignature(KeyHelper.withoutHeader(identityKey), signedPreKey.keyPair()
            .encodedPublicKey(), signedPreKey.signature()), "Signature mismatch",
  ...
  public void createIncoming(Session session, SignalPreKeyMessage message) {
    ...
    var nextState = createState(false, preKeyPair != null ?
            ...
            null, signedPreKeyPair.toGenericKeyPair(), message.identityKey(), message.baseKey(), null,
        message.registrationId(), message.version());
    session.addState(nextState);
  }

  public SessionState createState(boolean isInitiator, SignalKeyPair ourEphemeralKey,
    ...
    var signedIdentitySecret = Curve25519.sharedKey(KeyHelper.withoutHeader(theirSignedPubKey),
        ourSignedKey.privateKey());
    var ephemeralSecret = theirEphemeralPubKey == null || ourEphemeralKey == null ?
        null :
        Curve25519.sharedKey(KeyHelper.withoutHeader(theirEphemeralPubKey),
    ...
    var state = createState(isInitiator, ourEphemeralKey, ourSignedKey, theirIdentityPubKey,
        ...
        theirSignedPubKey, registrationId, version, masterKey);
    return isInitiator ?
        calculateSendingRatchet(state, theirSignedPubKey) :
        state;
  }
  ...
  private SessionState calculateSendingRatchet(SessionState state, byte[] theirSignedPubKey) {
    ...
    var key = state.ephemeralKeyPair()
        .encodedPublicKey();
  ...
  private SessionState createState(boolean isInitiator, SignalKeyPair ourEphemeralKey,
      SignalKeyPair ourSignedKey,
      byte[] theirIdentityPubKey, byte[] theirEphemeralPubKey, byte[] theirSignedPubKey,
      int registrationId,
      int version, byte[][] masterKey) {
    return SessionState.builder()
        ...
        .baseKey(isInitiator ?
            ourEphemeralKey.encodedPublicKey() :
            theirEphemeralPubKey)
        .closed(false)
        .build();
  }
