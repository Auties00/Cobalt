...
public class AesGmc {
  ...
  public byte[] decrypt(byte @NonNull [] iv, byte @NonNull [] input, byte @NonNull [] key,
      byte[] additionalData) {
    return cipher(iv, input, key, additionalData, false);
  }

  private byte[] cipher(byte @NonNull [] iv, byte @NonNull [] input, byte @NonNull [] key,
      byte[] additionalData,
      boolean encrypt) {
    try {
      ...
      throw new AesException(
          "Cannot %s data using AesGMC".formatted(encrypt ? "encrypt" : "decrypt"), exception);
    }
  }
