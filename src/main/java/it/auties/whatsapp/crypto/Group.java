...
public record Group(SenderKeyName senderKeyId, WhatsappKeys keys) {
  ...
  public SenderKeyDistributionMessage create(SenderKeyName senderKeyName) {
    ...
    if (senderKeyRecord.isEmpty()) {
      senderKeyRecord.senderKeyState(it.auties.whatsapp.crypto.Cipher.randomSenderKeyId(), 0, it.auties.whatsapp.crypto.Cipher.randomSenderKey(), it.auties.whatsapp.crypto.Cipher.randomKeyPair().privateKey());
  ...
  public byte[] decipher(byte[] senderKeyMessageBytes) {
    return decipher(senderKeyMessageBytes, ignored -> {});
  }

  public byte[] decipher(byte[] senderKeyMessageBytes, Consumer<byte[]> callback)  {
    var record = keys.findSenderKeyByName(senderKeyId);
  ...
  private SenderMessageKey senderKey(SenderKeyState senderKeyState, int iteration) {
    ...
    if (senderChainKey.iteration() > iteration) {
      Validate.isTrue(senderKeyState.hasSenderMessageKey(iteration),
              "Received message with old counter: %s,%s".formatted(senderChainKey.iteration(), iteration));
      return senderKeyState.removeSenderMessageKey(iteration);
    }

    Validate.isTrue(iteration - senderChainKey.iteration() <= MAX_MESSAGE_KEYS,
            "Message overflow: %s messages over the %s limit", MAX_MESSAGE_KEYS - iteration - senderChainKey.iteration(), MAX_MESSAGE_KEYS);
