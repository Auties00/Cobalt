...
abstract class FileWhatsappStoreSerializer implements WhatsappStoreSerializer {
    ...
    FileWhatsappStoreSerializer() {
        this(DEFAULT_SERIALIZER_PATH);
    }

    FileWhatsappStoreSerializer(Path baseDirectory) {
    ...
    abstract void encodeStore(WhatsappStore store, Path path);
    ...
    abstract WhatsappStore decodeStore(Path store) throws IOException;
    ...
    public void serializeStore(WhatsappStore store) {
        ...
        try {
            ...
            try(var executor = newVirtualThreadPerTaskExecutor()) {
                ...
                store.phoneNumber()
                        .ifPresent(phoneNumber -> executor.submit(() -> linkPhoneNumber(store.clientType(), store.uuid(), phoneNumber)));
            }
        } finally {
