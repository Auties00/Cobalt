 ...
 * <p>
 * This interface provides a standardized mechanism for serializing and deserializing WhatsApp
 * session components, including cryptographic keys ({@link Keys}) and session state ({@link WhatsappStore}).
 * Together, these components form a complete session that can be persisted across application restarts.
 * <p>
 ...
 * </ul>
 * <p>
 * Sessions are identified by either a UUID or a phone number. The serializer manages the lifecycle
 * of session data, including creation, retrieval, updating, and deletion.
 * <p>
 ...
 * @see Keys
 * @see StoreKeysPair
 * @see ProtobufStoreSerializer
 * @see DiscardingStoreSerializer
 */
...
public interface WhatsappStoreSerializer {
    ...
    /**
     * Creates a new session with fresh cryptographic keys and an initialized store.
     * <p>
     * This method initializes a complete session pair containing:
     * <ul>
     *     <li>A {@link WhatsappStore} with the provided UUID, phone number, and client type</li>
     *     <li>A {@link Keys} instance with newly generated cryptographic material</li>
     * </ul>
     * Both components are immediately serialized to persistent storage and configured to use
     * this serializer for future updates.
     *
     * @param uuid        the unique identifier for this session; must not be null
     * @param phoneNumber the phone number associated with this session; may be null for web sessions
     * @param clientType  the type of WhatsApp client (web or mobile); must not be null
     * @return a new {@link StoreKeysPair} containing the initialized store and keys
     */
    default StoreKeysPair newStoreKeysPair(UUID uuid, Long phoneNumber, WhatsappClientType clientType) {
        var store = WhatsappStore.of(uuid, phoneNumber, clientType);
        store.setSerializer(this);
        serializeStore(store);
        var keys = Keys.of(uuid, phoneNumber, clientType);
     ...
     * </ol>
     * If a matching session is found, both the {@link WhatsappStore} and {@link Keys} must exist for the
     * session to be considered valid. The returned components are configured to use this serializer
     * for future updates.
     *
     * @param uuid        the unique identifier of the session to retrieve; may be null
    ...
    default Optional<StoreKeysPair> deserializeStoreKeysPair(UUID uuid, Long phoneNumber, WhatsappClientType clientType) {
        if (uuid != null) {
    ...
    private Optional<StoreKeysPair> createStoreKeysPair(WhatsappStore store, Keys keys) {
     ...
     * insertion order (typically the order in which sessions were created).
     *
    ...
    /**
     * Persists cryptographic keys to storage.
     * <p>
     * This method serializes the provided {@link Keys} instance to persistent storage,
     * overwriting any existing keys for the same session. Implementations should ensure
     * that the serialization is atomic to prevent data corruption in case of interruption.
     *
     * @param keys the keys to serialize; must not be null
     */
     ...
     */
    void serializeStore(WhatsappStore store);

    /**
     * Retrieves cryptographic keys from storage by UUID.
     * <p>
     * This method deserializes a {@link Keys} instance from persistent storage using
     * the session's UUID as the identifier. The deserialization should be complete
     * and blocking.
     *
     * @param type the WhatsApp client type; must not be null
     * @param id   the UUID of the session whose keys should be retrieved; must not be null
     * @return an {@link Optional} containing the keys if found, or empty if no keys exist for this UUID
     */
    Optional<Keys> deserializeKeys(WhatsappClientType type, UUID id);

    /**
     * Retrieves cryptographic keys from storage by phone number.
     * <p>
     * This method deserializes a {@link Keys} instance from persistent storage using
     * the session's phone number as the identifier. The deserialization should be complete
     * and blocking.
     *
     * @param type        the WhatsApp client type; must not be null
     * @param phoneNumber the phone number of the session whose keys should be retrieved; must not be null
     * @return an {@link Optional} containing the keys if found, or empty if no keys exist for this phone number
     */
    Optional<Keys> deserializeKeys(WhatsappClientType type, Long phoneNumber);
     ...
     * @see #finishDeserializeStore(WhatsappStore)
     */
    Optional<WhatsappStore> deserializeStore(WhatsappClientType type, UUID id);
     ...
     * @see #finishDeserializeStore(WhatsappStore)
     */
    Optional<WhatsappStore> deserializeStore(WhatsappClientType type, Long phoneNumber);

    /**
     * Permanently removes a session from storage.
     * <p>
     * This method deletes all persistent data associated with the specified session,
     * including both the {@link WhatsappStore} and {@link Keys}. After this operation completes,
     * the session cannot be recovered and a new session must be created to use WhatsApp again.
     * <p>
     ...
     * <p>
     * This method should be called after {@link #deserializeStore(WhatsappClientType, UUID)} or
     * {@link #deserializeStore(WhatsappClientType, Long)} when the caller needs to ensure that
     ...
     */
    void finishDeserializeStore(WhatsappStore store);
}