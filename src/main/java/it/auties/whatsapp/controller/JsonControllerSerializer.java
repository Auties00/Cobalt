...
class JsonControllerSerializer implements ControllerSerializer {
    ...
    private static final String STORE_NAME = "store" + FILE_EXTENSION;
    private static final String KEYS_NAME = "keys" + FILE_EXTENSION;
    ...
    private CompletableFuture<Void> serializeChatAsync(Store store, Chat chat) {
        ...
        var fileName = CHAT_PREFIX + chat.jid().user() + FILE_EXTENSION;
    ...
    private CompletableFuture<Void> serializeNewsletterAsync(Store store, Newsletter newsletter) {
        var fileName = NEWSLETTER_PREFIX + newsletter.jid().user() + FILE_EXTENSION;
        var outputFile = getSessionFile(store, fileName);
    ...
    private void writeFile(byte[] object, String fileName, Path outputFile) {
        try {
            ...
            try (var tempFileOutputStream = Files.newOutputStream(tempFile)) {
    ...
    private Optional<Keys> deserializeKeysFromId(ClientType type, String id) {
        var path = getSessionFile(type, id, KEYS_NAME);
        try (var input = Files.newInputStream(path)) {
            return Optional.of(Json.readValue(input.readAllBytes(), Keys.class));
        } catch (IOException exception) {
            return Optional.empty();
        }
    }
    ...
    private Optional<Store> deserializeStoreFromId(ClientType type, String id) {
        var path = getSessionFile(type, id, STORE_NAME);
        if (Files.notExists(path)) {
            return Optional.empty();
        }

        try (var input = Files.newInputStream(path)) {
            return Optional.of(Json.readValue(input.readAllBytes(), Store.class));
        } catch (IOException exception) {
            return Optional.empty();
        }
    }
    ...
    private void deserializeChat(Store store, Path chatFile) {
        try (var input = Files.newInputStream(chatFile)) {
            var chat = Json.readValue(input.readAllBytes(), Chat.class);
    ...
    private void deserializeNewsletter(Store store, Path newsletterFile) {
        try (var input = Files.newInputStream(newsletterFile)) {
            var newsletter = Json.readValue(input.readAllBytes(), Newsletter.class);
