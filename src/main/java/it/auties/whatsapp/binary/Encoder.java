...
import static it.auties.whatsapp.binary.Tag.BINARY_20;
import static it.auties.whatsapp.binary.Tag.BINARY_32;
import static it.auties.whatsapp.binary.Tag.BINARY_8;
import static it.auties.whatsapp.binary.Tag.COMPANION_JID;
import static it.auties.whatsapp.binary.Tag.DICTIONARY_0;
import static it.auties.whatsapp.binary.Tag.DICTIONARY_1;
import static it.auties.whatsapp.binary.Tag.DICTIONARY_2;
import static it.auties.whatsapp.binary.Tag.DICTIONARY_3;
import static it.auties.whatsapp.binary.Tag.HEX_8;
import static it.auties.whatsapp.binary.Tag.JID_PAIR;
import static it.auties.whatsapp.binary.Tag.LIST_16;
import static it.auties.whatsapp.binary.Tag.LIST_8;
import static it.auties.whatsapp.binary.Tag.LIST_EMPTY;
import static it.auties.whatsapp.binary.Tag.NIBBLE_8;
...
public class Encoder {
  ...
  private int getStringCodePoint(Tag token, int codePoint) {
    ...
    throw new IllegalArgumentException(
        "Cannot parse codepoint %s with token %s".formatted(codePoint, token));
  }
  ...
  private Tag doubleByteStringTag(int index) {
    return switch (index / (Tokens.DOUBLE_BYTE.size() / 4)) {
      ...
      case 3 -> DICTIONARY_3;
      default ->
          throw new IllegalArgumentException("Cannot find tag for quadrant %s".formatted(index));
    };
  }
  ...
  private void write(Object input) {
    switch (input) {
      ...
      default -> throw new IllegalArgumentException(
          "Invalid payload type(%s): %s".formatted(input.getClass()
                  .getName(),
              input));
    }
  }
