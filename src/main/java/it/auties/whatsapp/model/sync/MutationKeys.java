...
public record MutationKeys(byte[] indexKey, byte[] encKey, byte[] macKey, byte[] snapshotMacKey, byte[] patchMacKey)
    ...
    public static MutationKeys of(byte @NonNull [] key) {
        ...
        var indexKey = buffer.cut(KEY_LENGTH)
                .toByteArray();
        var encKey = buffer.slice(KEY_LENGTH, KEY_LENGTH * 2)
                .toByteArray();
        var macKey = buffer.slice(KEY_LENGTH * 2, KEY_LENGTH * 3)
                .toByteArray();
        var snapshotMacKey = buffer.slice(KEY_LENGTH * 3, KEY_LENGTH * 4)
                .toByteArray();
        var patchMacKey = buffer.slice(KEY_LENGTH * 4)
                .toByteArray();
