...
public record MessageIndexInfo(@NonNull String type, @NonNull Optional<ContactJid> chatJid,
                               @NonNull Optional<String> messageId, boolean fromMe)
    implements Info, JacksonProvider {
   ...
   */
  public static MessageIndexInfo of(@NonNull String type, ContactJid chatJid, String messageId,
      boolean fromMe) {
    return new MessageIndexInfo(type, Optional.ofNullable(chatJid), Optional.ofNullable(messageId),
        fromMe);
  }
  ...
  public static MessageIndexInfo ofJson(@NonNull String json) {
    try {
      ...
      var type = getProperty(array, 0).orElseThrow(
          () -> new NoSuchElementException("Cannot parse MessageSync: missing type"));
      ...
      throw new IllegalStateException(
          "Cannot parse MessageSync: a json exception occurred while parsing %s",
          exception);
    }
  }
