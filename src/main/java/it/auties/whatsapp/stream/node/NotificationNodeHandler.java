...
import it.auties.whatsapp.io.json.response.NewsletterLeaveResponse;
import it.auties.whatsapp.io.json.response.NewsletterMuteResponse;
import it.auties.whatsapp.io.json.response.NewsletterResponse;
import it.auties.whatsapp.io.json.response.NewsletterStateResponse;
...
public final class NotificationNodeHandler extends AbstractNodeHandler {
    ...
    public NotificationNodeHandler(Whatsapp whatsapp, PhonePairingCode pairingCode) {
    ...
    private void handleNewsletter(Node node) {
        ...
        for (var messageNode : messages.get().listChildren("message")) {
            ...
            var newsletterMessage = whatsapp.store().findMessageById(newsletter.get(), messageId);
    ...
    private void handleCompanionRegistration(Node node) {
        try {
            ...
            var companionSharedKey = Curve25519.sharedKey(codePairingPublicKey, whatsapp.keys().companionKeyPair().privateKey().toEncodedPoint());
            ...
            var identitySharedKey = Curve25519.sharedKey(primaryIdentityPublicKey, whatsapp.keys().identityKeyPair().privateKey().toEncodedPoint());
            ...
            var companionPrivateKey = whatsapp.keys().companionKeyPair().privateKey();
