...
import it.auties.whatsapp.registration.SSLProvider;
...
public class ApnsClient implements AutoCloseable {
    ...
    private void createSocketConnection(ApnsBag bag) {
      try {
          var sslContext = SSLProvider.getSslContext(3, true);
    ...
    private int getPayloadItemsCount(Object entry) {
        return switch (entry) {
            case byte[] bytes -> 1;
    ...
    private record BagSignature(byte[] signature, byte[][] certificates, byte[] bag) {
