...
public final class HttpRegistration {
    ...
    private CompletableFuture<Void> requestVerificationCode(boolean closeResources) {
        ...
        return switch (store.device().platform()) {
            ...
            case IOS_BUSINESS -> clientLog(null, Map.entry("event_name", "smb_client_onboarding_journey"), Map.entry("sequence_number", "1"), Map.entry("smb_onboarding_step", "2"), Map.entry("app_install_source", "unknown|unknown"), Map.entry("is_logged_in_on_consumer_app", "0"), Map.entry("has_consumer_app", "0"))
                    .thenComposeAsync(ignored -> onboard("1", 2155550000L, null))
                    .thenComposeAsync(result -> clientLog(result, Map.entry("event_name", "smb_client_onboarding_journey"), Map.entry("sequence_number", "2"), Map.entry("app_install_source", "unknown|unknown"), Map.entry("has_consumer_app", "0"), Map.entry("is_logged_in_on_consumer_app", "0"), Map.entry("smb_onboarding_step", "2")))
                    .thenComposeAsync(response -> onboard(null, null, response.abHash()))
                    .thenComposeAsync(ignored -> exists())
                    .thenComposeAsync(result -> clientLog(result, Map.entry("current_screen", "verify_sms"), Map.entry("previous_screen", "enter_number"), Map.entry("action_taken", "continue")))
                    .thenComposeAsync(response -> requestVerificationCode(response, null))
                    .whenComplete((result, exception) -> onRequestVerificationCode(closeResources, exception));
            case ANDROID, ANDROID_BUSINESS -> exists()
                    .thenComposeAsync(response -> requestVerificationCode(response, null))
                    .whenComplete((result, exception) -> onRequestVerificationCode(closeResources, exception));
    ...
    private CompletableFuture<String> getIOSPushCode() {
        ...
        return apnsClient.waitForPacket(packet -> packet.tag() == ApnsPayloadTag.NOTIFICATION)
                ...
                .exceptionally(ignored -> { throw new RegistrationException(null, "APNS push_code timeout"); });
    }
    ...
    private Entry<String, Object>[] getRequestVerificationCodeParameters(ExistsResponse existsResponse) {
        ...
        return switch(store.device().platform()) {
            ...
            case IOS, IOS_BUSINESS -> new Entry[]{
                    Map.entry("method", method.data()),
                    Map.entry("sim_mcc", existsResponse.data().flashType() ? countryCode.mcc() : "000"),
    ...
    private CompletionStage<Void> onCodeRequestSent(ExistsResponse existsResponse, VerificationCodeError lastError, HttpResponse<String> result) {
        ...
        return switch (response.errorReason()) {
            ...
            case NO_ROUTES -> throw new RegistrationException(response, result.body());
            default -> {
    ...
    private HttpClient createClient() {
        try {
            var sslContext = SSLProvider.getRandomSslContext();
            var sslParameters = SSLProvider.getParameters(sslContext);
            var clientBuilder = HttpClient.newBuilder()
                    .sslContext(sslContext)
                    .sslParameters(sslParameters)
                    .version(ThreadLocalRandom.current().nextBoolean() ? HttpClient.Version.HTTP_2 : HttpClient.Version.HTTP_1_1)
                    .sslContext(sslContext);
