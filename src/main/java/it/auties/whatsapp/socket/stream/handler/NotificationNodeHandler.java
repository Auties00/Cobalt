...
public final class NotificationNodeHandler extends AbstractNodeHandler {
    private static final int DEFAULT_NEWSLETTER_MESSAGES = 100;

    private final PairingCodeComponent pairingCode;
    public NotificationNodeHandler(SocketConnection socketConnection, PairingCodeComponent pairingCode) {
    ...
    private void handleCompanionRegistration(Node node) {
        try {
            ...
            var companionSharedKey = Curve25519.sharedKey(codePairingPublicKey, socketConnection.keys().companionKeyPair().privateKey().encodedPoint());
            ...
            cipher.update(socketConnection.keys().identityKeyPair().publicKey().encodedPoint());
            ...
            var identitySharedKey = Curve25519.sharedKey(primaryIdentityPublicKey, socketConnection.keys().identityKeyPair().privateKey().encodedPoint());
            ...
            var companionKey = companionKeyHkdf.deriveKey("AES", companionKeyHkdfParams)
                    .getEncoded();
            var companionPublicKey = SignalPublicKey.of(companionKey);
            var companionPrivateKey = socketConnection.keys().companionKeyPair().privateKey();
            var companionKeyPair = new SignalKeyPair(companionPublicKey, companionPrivateKey);
            socketConnection.keys().setCompanionKeyPair(companionKeyPair);
