...
public final class WebAppStateComponent {
    ...
    public WebAppStateComponent(SocketConnection socketConnection) {
    ...
    private Optional<ActionDataSync> decodeMutation(Jid jid, RecordSync.Operation operation, RecordSync sync, LTHash generator) {
        try {
            ...
            if(socketConnection.store().checkPatchMacs()) {
                var expectedMac = generateMac(operation, blob, blob.length - SignalPublicKey.length(), sync.keyId().id(), mutationKeys.get().macKey());
                if(!Arrays.equals(
                        blob, blob.length - SignalPublicKey.length(), blob.length,
                        expectedMac, 0, SignalPublicKey.length()
                )) {
            ...
            var result = cipher.doFinal(blob, 16, blob.length - SignalPublicKey.length() - 16);
            ...
            generator.mix(sync.index().blob(), Arrays.copyOfRange(blob, blob.length - SignalPublicKey.length(), blob.length), operation);
    ...
    private byte[] generateMac(RecordSync.Operation operation, byte[] data, int dataLength, byte[] keyId, byte[] key) {
        try {
            ...
            return Arrays.copyOfRange(sha512, 0, SignalPublicKey.length());
        }catch (GeneralSecurityException exception) {
        ...
        return getMutationKeys(jid, patch.keyId()).map(mutationKeys -> {
            try {
                ...
                for(var mutation : patch.mutations()) {
                    ...
                    localMac.update(blob, blob.length - SignalPublicKey.length(), SignalPublicKey.length());
                }
