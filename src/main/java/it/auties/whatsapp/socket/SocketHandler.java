...
public class SocketHandler extends Handler
  ...
  public CompletableFuture<Node> send(Node node, Function<Node, Boolean> filter) {
    ...
    onNodeSent(node);
    return node.toRequest(node.id() == null ? store.nextTag() : null, filter)
  ...
  public CompletableFuture<Void> sendQueryWithNoResponse(String method, String category,
      Node... body) {
    return sendQueryWithNoResponse(null, Server.WHATSAPP.toJid(), method, category, null, body);
  }

  public CompletableFuture<Node> sendQuery(String method, String category,
      Map<String, Object> metadata,
      Node... body) {
    return sendQuery(null, Server.WHATSAPP.toJid(), method, category, metadata, body);
  }

  public CompletableFuture<Node> sendQuery(ContactJid to, String method, String category,
      Node... body) {
  ...
  public CompletableFuture<Void> sendQueryWithNoResponse(String id, ContactJid to, String method,
      String category,
      Map<String, Object> metadata, Node... body) {
    var attributes = Attributes.ofNullable(metadata)
        .put("id", id, Objects::nonNull)
  ...
  public CompletableFuture<Optional<URI>> queryPicture(@NonNull ContactJidProvider chat) {
    ...
    return sendQuery("get", "w:profile:picture", Map.of("target", chat.toJid()),
        body).thenApplyAsync(
  ...
  public void sendReceipt(ContactJid jid, ContactJid participant, List<String> messages,
      String type) {
    if (messages.isEmpty()) {
      return;
    }
  ...
  protected void onMessageStatus(MessageStatus status, Contact participant, MessageInfo message,
      Chat chat) {
    callListenersAsync(listener -> {
      if (participant == null) {
  ...
  public void updateUserName(String newName, String oldName) {
    if (oldName != null && !Objects.equals(newName, oldName)) {
      ...
      sendWithNoResponse(
          Node.ofAttributes("presence", Map.of("name", newName, "type", "available")));
      callListenersAsync(listener -> {
