...
/**
 * Builds the Facebook GraphQL query that fetches the eligible quick-promotion banners for a set of WhatsApp
 * Business surfaces.
 *
 ...
 * {@code country} short-code, and the user {@code locale}. The Meta graph endpoint also pins two server-side literal
 * arguments ({@code include_holdouts} and {@code supports_client_side_filters}, both {@code true}) that
 * are baked into the persisted document and carry no client variable. The Meta graph endpoint returns one
 * surface-keyed batch under {@code quick_promotion_batch_fetch_root}; the reply is consumed through
 * {@link FetchQuickPromotionsFacebookGraphQlResponse}.
 *
 * @see FetchQuickPromotionsFacebookGraphQlResponse
 */
@WhatsAppWebModule(moduleName = "WAWebFetchQuickPromotionsQuery")
public final class FetchQuickPromotionsFacebookGraphQlRequest implements FacebookGraphQlOperation.Request {
    /**
     ...
     */
    public FetchQuickPromotionsFacebookGraphQlRequest(List<String> nuxIds, TriggerContext triggerContext) {
        this.nuxIds = nuxIds;
    ...
    public record TriggerContext(Boolean isFromWaSmb, String appVersion, String country, String locale) {
         ...
         *
         * <p>Reserved for {@link FetchQuickPromotionsFacebookGraphQlRequest#variables()}; writes each field only
