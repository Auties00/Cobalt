...
/**
 * Builds the Facebook GraphQL request that fetches the caller's OIDC sign-in state for the WhatsApp Business
 * platform.
 *
 * <p>The operation takes no variables; it asks the Meta graph endpoint for the current OpenID Connect state blob
 * tied to the authenticated account, returned verbatim as the scalar
 * {@code xfb_wa_biz_get_oidc_state}. The reply is consumed through {@link FetchOidcStateFacebookGraphQlResponse}.
 *
 * @see FetchOidcStateFacebookGraphQlResponse
 */
@WhatsAppWebModule(moduleName = "WAWebFetchOIDCStateQuery")
public final class FetchOidcStateFacebookGraphQlRequest implements FacebookGraphQlOperation.Request {
    /**
     ...
     */
    public FetchOidcStateFacebookGraphQlRequest() {
    }
