#!/usr/bin/env node
// Splits ./history/_raw/bootstrap-dump.json (one JSON document with
// notifications + chunks captured from a live WA Web session via the
// __hs_capture IDB hook) into per-syncType fixture files under ./history/:
//
//   ./history/<typeSlug>/notification.json   (HistorySyncNotification proto json)
//   ./history/<typeSlug>/chunk.b64           (inflated HistorySync proto bytes, base64)
//   ./history/<typeSlug>/expected.json       (decoded HistorySync proto value)
//
// Notification + chunk events are paired by their adjacent timestamps.
// Run from this directory: node split-history.mjs
...
const DUMP = resolve(SCRIPT_DIR, 'history/_raw/bootstrap-dump.json');
const OUT_ROOT = resolve(SCRIPT_DIR, 'history');
