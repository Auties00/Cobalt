...
// The define hook records every defined module name and, for modules that look
// TOS-related, captures the factory function source so the extractor can parse
// notice-id definitions without calling the getters (which can have side effects).
const DEFINE_HOOK = `(() => {
  if (window.__waDefineHookInstalled) return;
  window.__waDefineHookInstalled = true;
  window.__waDefinedModules = [];
  window.__waModuleSources = {};
  const isTosRelated = (src) =>
    (src.indexOf("Tos") !== -1 || src.indexOf("Notice") !== -1 || src.indexOf("Nux") !== -1) &&
    (src.indexOf("getABPropConfigValue") !== -1 ||
     src.indexOf("TOS_STATE_") !== -1 ||
     src.indexOf("TosManager") !== -1);
  const record = (name, args) => {
    try {
      window.__waDefinedModules.push(name);
      for (let i = 1; i < args.length; i++) {
        if (typeof args[i] === "function") {
          const src = args[i].toString();
          if (isTosRelated(src)) window.__waModuleSources[name] = src;
          break;
        }
      }
    } catch (e) {}
...
export async function withForceLoadedBundle<T>(
  ...
  try {
    ...
    const tosModuleCount = await page.evaluate(
      "Object.keys(window.__waModuleSources || {}).length",
    );
    console.log(`[INFO] ${moduleCount} modules defined; ${tosModuleCount} TOS-related captured; extracting...`);
